...
public class OrderDao {
	public static void exportOrderList(HttpServletResponse response,int time,	int type, int state, String ordercode,int ishas, String receiver, String time_a, String time_b, String aid, String tel){
		List<Record> list = MCDao.getOrderList(1, 100000000, time, type, state, ordercode, ishas, receiver, time_a, time_b, aid, tel).getList();
       ...
       //首行 样式2
		...
		for(int i=0;i<list.size();i++){
			...
			cell6.setEncoding(HSSFCell.ENCODING_UTF_16);
			cell6.setCellValue(reachName);
	
			HSSFCell cell7 = row.createCell((short) 7);
			cell7.setCellStyle(cellStyle1);
			cell7.setCellValue(r.getDouble("totalprice"));
			...
			cell8.setEncoding(HSSFCell.ENCODING_UTF_16);
			cell8.setCellValue(ordertype);
			
			HSSFCell cell9 = row.createCell((short) 9);
			cell9.setCellStyle(cellStyle1);
			Timestamp timestamp = r.getTimestamp("ctime");
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			cell9.setCellValue(sdf.format(timestamp));
			
			HSSFCell cell10 = row.createCell((short) 10);
			cell10.setCellStyle(cellStyle1);
			cell10.setEncoding(HSSFCell.ENCODING_UTF_16);
			cell10.setCellValue(Constant.zhbyos(r.getInt("state")));
